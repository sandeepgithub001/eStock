...
@Component({
  selector: 'app-monthly-leave-factor',
  templateUrl: './monthly-leave-factor.component.html',
  styleUrls: ['./monthly-leave-factor.component.css']
})
export class MonthlyLeaveFactorComponent implements OnInit {
  leaveFactorId = 400001;
  searchKey: string = "";
  statusId: string = "1";
  ...
  onEditRow(ev, index) {
    ...
    if (!this.leaveFactors[index].isEditable) {
      ...
      // if (parseFloat(this.leaveFactors[index].FactorValue) < 0 || isNaN(parseFloat(this.leaveFactors[index].FactorValue))) {
      //   this.globalService.showNotification("Please enter a valid Factor");
      //   return;
      // }
